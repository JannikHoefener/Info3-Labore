...
unsigned int wTime;
unsigned int pTime;
unsigned int timer = 100; // Count Down für Timer Funktion
...
ISR(TIMER0_COMPA_vect)
{
	...
	// Einmal pro Sekunde wir der Wert um eins gesenkt, wenn der Timer
	// abgelaufen ist, schaut die Funktion in einem Switch nach, was entsprechend
	// des aktuellen States passieren soll.
	timer--;
	if (timer == 0) {
		switch(state) {
			case 3: // State Timer Phase
				// Timer Phase abgelaufen
				// Buzzer High setzen
				timer = 1;
				state++;
				break;
			case 4: // State Buzzer Signal
				// Buzzer Low setzen
				timer = pTime; // Dauer der Pause in den Timer schreiben
				state++;
				break;
...
void displayTimer(int restzeit) {
	TFT_Print(restzeit, 25, 44, 4, TFT_16BitBlack, TFT_16BitWhite, TFT_Landscape180);
...
void main(void){
	...
	// Messwert zu Sekunden umrechnen
	int sekunden = 1500;
	...
	// timer mit der Dauer der Arbeitsphase füllen
	// State erhöhen, Timer einschalten und ab gehts.
	timer = wTime;
	state++;
